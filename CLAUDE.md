...
- `RUST_LOG=debug cargo run` — подробные логи (`tracing-subscriber` `EnvFilter`, дефолт `INFO`).
...
- `HidHello=0xBB` — host liveness ping (общий для обеих ОС).
...
- `HidKbState=0xDD` — поток с устройства на хост; разбирается в `StateProvider` (только этот дискриминант, остальные кадры провайдер игнорирует) и логируется через `tracing::info!` (`RUST_LOG=info`). Подтипы в `HidKbStateSubtype`: `Layer=1`, `Lang=2`, `MacMode=3`, `RuenLayout=4`.
